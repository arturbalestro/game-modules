...
class UndergroundPath extends React.Component {
  constructor(props) {
    super(props);
  ...
  getAvailablePokemon() {
    ...
    const availablePokemon = wildGroup[0].node.pokemons.edges.filter(function(pokemon) {
      return pokemon.node.pokemonType === "Ground"
          || pokemon.node.pokemonType === "Poison";
    });
  ...
  render() {
    ...
    return (
      <div className="underground-path">
        <h1>Underground Path</h1>
        <h2>{headerText}</h2>
        {this.generateTiles()}
...
export function path() {
  return '/underground-path';
}

export default Relay.createContainer(UndergroundPath, {
  fragments: {
    game: () => Relay.QL`
      fragment on Game {
        id,
