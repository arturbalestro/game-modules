...
class ViridianForest extends React.Component {
  constructor(props) {
    super(props);
  ...
  getAvailablePokemon() {
    ...
    const availablePokemon = wildGroup[0].node.pokemons.edges.filter(function(pokemon) {
      return pokemon.node.pokemonType === "Bug";
    });
...
export default Relay.createContainer(ViridianForest, {
  fragments: {
    game: () => Relay.QL`
      fragment on Game {
        id,
